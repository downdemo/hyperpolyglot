    ...
    if (element == CE_TabBarTabLabel) {
      if (const QStyleOptionTab *tab =
        ...
        if (tab->state & QStyle::State_Selected) {
          painter->save();
          painter->setPen(0xFFDEAD);            // tab 边框线颜色
          painter->setBrush(QBrush(0x6A5ACD));  // tab 背景色
          painter->drawRect(allRect.adjusted(0, 0, -1, -1));  // 调整边距
          painter->restore();
          painter->setPen(0xFAFFF0);  // 选中 tab 的文字颜色
        } else {
          painter->setPen(0x6A5ACD);            // 未选中 tab 的文字颜色
          painter->setBrush(QBrush(0xF8F8FF));  // 未选中 tab 的背景色
          painter->drawRect(allRect.adjusted(0, 0, -1, -1));  // 调整边距
        }
