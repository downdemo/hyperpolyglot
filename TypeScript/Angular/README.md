...
```sh
cd demo
ng serve -o // -o 选项表示编译完成后自动打开 http://localhost:4200/
```
...
```sh
ng serve -o --port 4396 // 通过 http://localhost:4396/ 访问
```

* 如果想让局域网中的其他用户访问项目，通过 `--host` 选项指定本机 ip 地址即可

```sh
ng serve --host 192.168.0.1 // 局域网中用户均能通过 http://192.168.0.1:4200 访问
```
